...
const server = http.createServer(async (req, res) => {
  ...
  // Optional same-origin proxy; credentials remain in the request, never in files.
  if (url.pathname.startsWith('/api/landing/')) {
    res.setHeader('Content-Type', 'application/json; charset=utf-8');
    res.setHeader('Cache-Control', 'no-store');
    const base = process.env.LANDING_API_URL;
    if (!base) { res.writeHead(503); res.end(JSON.stringify({error:'La publicación aún no está conectada. Configura LANDING_API_URL en el servidor.'})); return; }
    try {
      const target = new URL(base.replace(/\/$/, '') + url.pathname.slice('/api/landing'.length));
      if (target.protocol !== 'https:' && !(target.protocol === 'http:' && ['127.0.0.1','localhost'].includes(target.hostname))) throw new Error('Invalid backend');
      const chunks = []; let size = 0;
      for await (const chunk of req) { size += chunk.length; if (size > 100000) { res.writeHead(413); res.end(JSON.stringify({error:'Solicitud demasiado grande.'})); return; } chunks.push(chunk); }
      const headers = { 'Content-Type':'application/json' };
      if (req.headers.authorization) headers.Authorization = req.headers.authorization;
      if (req.headers.origin) headers.Origin = req.headers.origin;
      const upstream = await fetch(target, { method:req.method, headers, ...(chunks.length ? {body:Buffer.concat(chunks)} : {}), signal:AbortSignal.timeout(65000), redirect:'error' });
      if (!upstream.headers.get('content-type')?.includes('application/json')) throw new Error('Invalid backend response');
      res.setHeader('Cache-Control',upstream.headers.get('cache-control') || 'no-store');
      res.writeHead(upstream.status); res.end(await upstream.text());
    } catch (_) { res.writeHead(503); res.end(JSON.stringify({error:'No se pudo conectar con la publicación. Vuelve a intentarlo.'})); }
    return;
  }
